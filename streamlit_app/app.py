...
_LANG_TO_EXT  = {"python": "py", "javascript": "js", "typescript": "ts",
                 "java": "java", "go": "go"}
...
# ── Page config ───────────────────────────────────────────────────────────────
st.set_page_config(page_title="LLM Code Review Agent", page_icon="🔍",
...
.finding-card { background:#151922; border:1px solid #262B36; border-left:4px solid var(--sev-color,#7C8B9C); border-radius:8px; padding:1rem 1.2rem; margin-bottom:1rem; }
.finding-header { display:flex; align-items:center; gap:0.6rem; margin-bottom:0.4rem; flex-wrap:wrap; }
.severity-badge { font-family:'JetBrains Mono',monospace; font-size:0.72rem; font-weight:600; letter-spacing:0.04em; padding:0.15rem 0.5rem; border-radius:4px; color:#0E1116; }
...
.code-block { font-family:'JetBrains Mono',monospace; font-size:0.82rem; padding:0.6rem 0.8rem; border-radius:6px; white-space:pre-wrap; word-break:break-word; margin-bottom:0.5rem; line-height:1.5; }
...
.clean-banner,.empty-banner { border-radius:8px; padding:0.9rem 1.1rem; font-family:'JetBrains Mono',monospace; font-size:0.9rem; }
...
.provider-badge { font-family:'JetBrains Mono',monospace; font-size:0.75rem; color:#6C8EF5; background:rgba(108,142,245,0.1); padding:0.2rem 0.6rem; border-radius:4px; display:inline-block; margin-bottom:0.8rem; }

.stButton button { background-color:#6C8EF5; color:#0E1116; font-weight:600; border-radius:6px; border:none; padding:0.5rem 1.4rem; }
...
st.markdown(
    '<div class="hero-sub">Finds bugs and security issues across Python, JavaScript, '
    'TypeScript, Java, and Go — grounded in OWASP guidance, with suggested fixes '
    'and plain-English explanations.</div>',
    unsafe_allow_html=True,
)
...
with st.sidebar:
    ...
    default_provider = os.getenv("MODEL_PROVIDER", "anthropic")
    ...
    selected_provider = st.selectbox(
        ...
        provider_keys,
        index=default_idx,
        format_func=lambda p: _PROVIDERS[p]["label"],
        label_visibility="collapsed",
    )

    pinfo         = _PROVIDERS[selected_provider]
    default_model = os.getenv(pinfo["env_key"], pinfo["models"][0])
    model_options = pinfo["models"] if default_model in pinfo["models"] else [default_model] + pinfo["models"]
...
st.markdown(
    f"<div class='provider-badge'>⚡ {pinfo['label']} — {final_model}</div>",
    unsafe_allow_html=True,
)
...
if run:
    if mode == "Paste code":
        ...
        files_payload = [{"path": f"pasted_snippet.{ext}", "content": code, "language": language}]
    else:
    ...
    with st.spinner(f"Analyzing with {pinfo['label']} ({final_model})..."):
        try:
            response = httpx.post(
                f"{API_URL}/review",
                json={"files": files_payload, "provider": selected_provider, "model": final_model},
                timeout=300,
            )
...
if findings is None:
    st.markdown(
        '<div class="empty-banner">$ waiting for input — paste code or a GitHub PR URL, then run a review.</div>',
        unsafe_allow_html=True,
    )
elif not findings:
    st.markdown(
    ...
    for file, file_findings in by_file.items():
        ...
        for f in sorted(file_findings, key=lambda x: x["line"]):
            ...
            cat   = html.escape(f["category"].replace("_", " ").title())
            card  = (
            ...
            snippet = f.get("original_snippet") or "// Code snippet unavailable."
            card += (
